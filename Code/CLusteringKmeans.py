...
def main():
    ...
    with open('IPLBowlers.csv', 'r') as f:
        for row in f:
            line=row.split(",")
            line[1]=line[1].strip("\n")
            points.append(makeRandomPoint(float(line[1]),float(line[1])))
            l = [float(line[1]),float(line[1])]
            userPoints[line[0]] = l
    ...
    for i, c in enumerate(clusters):
        for p in c.points:
           ...
           with open('Bowlers_classified.csv', 'a',newline='') as f:
               fieldnames = ['Cluster ID', 'Username', 'Value']
               writer = csv.DictWriter(f, fieldnames=fieldnames)
               for e in userPoints.values():
                    if str(p) == str(e):
                        if e not in u:
                            name = list(userPoints.keys())[list(userPoints.values()).index(e)]
                            u.append(e)
                            writer.writerow({'Cluster ID': i, 'Username': name, 'Value': str(p).strip()})
